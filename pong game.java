...
class Game extends JPanel implements ActionListener {
    ...
    public void move() {
        ...
        {
            if (bally == 0) { // if the ball is at the top of the board, make it move down
                ballup = false;
                balldown = true;
                ballx += 10;
                bally += 10;
            } else { // if the ball is not at the border of the board, move it diagonally
        ...
        if (ballright && balldown) { // if the ball is moving right and down
            if (bally == 600) { // if the ball is at the bottom of the board, make it move up
                ballup = true;
                balldown = false;
                ballx += 10;
                bally -= 10;
        ...
        if (ballleft && ballup) { // if the ball is moving left and up
            if (bally == 0) { // if the ball is at the top of the board, make it move down
                ballup = false;
                balldown = true;
                ballx -= 10;
    ...
    public void checkWins() {
        if (ballx <= 0) { // if the ball is at the left border of the board, the right player wins
            inGame = false;
            win = 1;
        }
        if (ballx >= 1100) { // if the ball is at the right border of the board, the left player wins
            inGame = false;
            win = 0;
        }
        if (ballx == rightx) { // if the ball is at the right paddle, make it move left
            if (bally >= righty && bally <= (righty + 100)) { // if the ball is at the right paddle, make it move left
                ballright = false;
                ballleft = true;
            }

        }
        if (ballx == leftx) { // if the ball is at the left paddle, make it move right
            if (bally >= lefty && bally <= (lefty + 100)) { // if the ball is at the left paddle, make it move right
                ballright = true;
                ballleft = false;
            }
        }
    ...
    public void doPaint(Graphics g) {
        if (inGame) { // if the game is in progress, paint the board
        ...
        } else { // if the game is not in progress, paint the winner
            gameOver(g);
        }
    ...
    public class KeyMover extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if ((key == KeyEvent.VK_P)) { // if the key pressed is P, move the right paddle up
                rightup = true;
                rightdown = false;
            }

            if ((key == KeyEvent.VK_L)) { // if the key pressed is L, move the right paddle down
                rightup = false;
                rightdown = true;
            }
